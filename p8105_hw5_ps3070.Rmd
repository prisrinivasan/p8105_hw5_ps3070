...
Next, I repeated this simulation for $\beta_1 = \{1, 2, 3, 4, 5, 6\}$. The table generated in the code chunk below returns estimates and p-values, simulated 10,000 times for each beta.
...
Below is a plot showing the proportion of times the null was rejected (power) on the y axis and the true value of $\beta_1$  on the x axis. As seen in this plot, as effect size increases, power increases. 
...
Plot to show the average estimate of $\hat{\beta}_1$ on y axis and the true value of $\beta_1$ on the x-axis
...
Plot to show the average estimate of $\hat{\beta}_1$ only where null was rejected (p < 0.05) on the y-axis and the true value of $\beta_1$ on the x-axis
...
```{r}
plot = overall_plot + rej_plot
```

Overlay:

#```{r}
ggplot(overall_plot, aes(aes(x = betas, y = mean_beta1))) +
  geom_point() + 
  geom_line() +
  geom_line(data = rej_plot, aes(x = betas, y = mean_rej_beta1), colour = "red")
```
